...
const dataDir = path.join(__dirname, "../../src/content/festivals/montreal-jazz-2026");

const ALGOLIA_URL = "https://montrealjazzfest.com/api/algolia/search";
const INDEX_NAME = "master_festival_FIDJM_fr-CA";
...
export async function run(): Promise<void> {
  console.log("[montreal-jazz-2026] Fetching schedule from Algolia API...");
  const shows = await fetchShows();
  console.log(`[montreal-jazz-2026] Got ${shows.length} shows`);
  ...
  for (const show of shows) {
    ...
    const venue = show.venues?.name?.trim() ?? "";
    const genre = show.genre?.[0]?.name;
    const artist = show.event_name.trim();
